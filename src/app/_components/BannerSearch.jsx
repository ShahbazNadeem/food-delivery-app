...
const BannerSearch = () => {
    ...
    const loadRestaurats = async () => {
        try {
            const res = await fetch('http://localhost:3000/api/customer?location=muree');
            const data = await res.json();
            if (data.success) {
    ...
    return (
        <>
            ...
            <section>
                <div className="">
                    <h2 className='text-center'>Restaurants near you</h2>
                    {
                        restaurants?.map((items) => {
                            return (
                                <ul key={items.id} className='flex gap-3'>
                                    <li>{items.name}</li>
                                    <li>{items.city}</li>
                                    <li>{items.contact}</li>
                                </ul>
